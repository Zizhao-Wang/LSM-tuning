...
for i in 13; do
    dir1="SATune_990PRO_MultiTwitterClusters_Benchmarking_Performance2"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            for cluster_a in "$i"; do  # 
                ...
                for workload_kvs in 100000000 ; do 
                    ...
                    for table_cache_size in ${cluster_table_size_map[$cluster_a]}; do
                        ...
                        # 创建相应的目录
                        db_dir="/mnt/nvm/SATune/PreLoad_Cluster${cluster_a}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                        if [ ! -d "$db_dir" ]; then
                            mkdir -p "$db_dir"
                        fi
