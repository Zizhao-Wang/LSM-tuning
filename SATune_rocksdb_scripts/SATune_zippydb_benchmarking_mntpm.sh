...
for i in 35; do
    dir1="SATune_990PRO_MultiTwitterClusters_Benchmarking_Performance22"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            for cluster_a in "$i"; do  # 
                ...
                for workload_kvs in 50000000 ; do 
                    ...
                    for table_cache_size in ${cluster_table_size_map[$cluster_a]}; do
                        ...
                        tuning_log_dir="$(pwd)"
                        log_file="SATune_${num_format2}_val${value_size_twitter}_mem${buffer_size_mb}MB_Cluster${cluster_a}_CT0${ct0}_L1${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}.log"
                        # data_file="/mnt/nvm/second_cluster${cluster_a}.sort" # 构建数据文件路径
                        data_file="/mnt/workloads/second_cluster${cluster_a}.sort" # 构建数据文件路径
                        memory_log_file="$(pwd)/SATune_Memory_${num_format2}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}MiB_CT0${ct0}_L1${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}.log"      


                        # 创建相应的目录
                        db_dir="/mnt/pmem1/SATune/PreLoad_Cluster${cluster_a}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                        if [ ! -d "$db_dir" ]; then
                            mkdir -p "$db_dir"
                        fi
                        ...
                        ../../SATunedb/release/db_bench \
                            ...
                            perf stat -p $DB_BENCH_PID 2>&1 | tee "SATune_perf_stat_${num_format2}in${num_format3}_val_${value_size_twitter}_Cluster${cluster_a}_mem${MEM}MiB_CT0${ct0}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}.txt" &
                            PERF_PID=$!
