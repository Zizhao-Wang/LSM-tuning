...
for i in {10..10}; do
  ...
  for value_size in 128; do
    ...
    for table_cache_size in 10000; do
      ...
      log_file="SATune_${num_format2}_in${num_format3}_val${value_size}_mem${buffer_size_mb}MB_zipf${zipf_a}_CT0${ct0}_level1base${mb}_targetbase${target_file_base_mb}_Blk${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_${iteration}.log"
      data_file="/mnt/nvm/zipf${zipf_a}_keys10.0B.csv" # 构建数据文件路径
      memory_log_file="$(pwd)/SATune_BenchMemory_${num_format2}in${num_format3}_key${key_siz}_val${value_size}_mem${buffer_size_mb}MiB_zipf${zipf_a}_CT0${ct0}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_${iteration}.log"      
      ...
      perf stat -p $DB_BENCH_PID 2>&1 | tee "SATune_perf_stat_${num_format2}in${num_format3}_val_${value_size}_zipf${zipf_a}_mem${MEM}MiB_CT0${ct0}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_${iteration}.txt" &
      PERF_PID=$!
