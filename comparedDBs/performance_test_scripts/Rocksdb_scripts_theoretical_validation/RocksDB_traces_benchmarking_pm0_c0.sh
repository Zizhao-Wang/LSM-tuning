...
for i in 40 51 35 ; do
    dir1="RocksDB_MultiTwitterClusters_Space_Performance_PM0"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            for cluster_a in "$i"; do  # 
                ...
                for workload_kvs in 100000000 ; do 
                    ...
                    for mu in 10; do
                        ...
                        # 创建相应的目录
                        db_dir="/mnt/pmem0/rocks10B/PreLoad_Cluster${cluster_a}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_f${mu}"
                        if [ ! -d "$db_dir" ]; then
                            mkdir -p "$db_dir"
                        fi
