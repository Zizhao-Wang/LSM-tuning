...
for i in 35; do
    dir1="RocksDB_SATASSD_MultiTwitterClusters_Benchmarking_Claude4.5"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            for cluster_a in "$i"; do  #  
                for workload_kvs in 100000000 ; do 
                    ...
                    for table_cache_size in ${cluster_table_size_map[$cluster_a]}; do
                        ...
                        log_file="RocksDB_${num_format2}_val${value_size_twitter}_mem${buffer_size_mb}MB_Cluster${cluster_a}_L1${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}iteration3.log"
                        # data_file="/mnt/nvm/second_cluster${cluster_a}.sort" # 构建数据文件路径
                        data_file="/mnt/workloads/second_cluster${cluster_a}.sort" # 构建数据文件路径
                        memory_log_file="$(pwd)/RocksDB_Memory_${num_format2}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}MiB_L1${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}iteration3.log"      
                        ...
                        # 创建相应的目录
                        db_dir="/mnt/db_test/rocks10B/PreLoad_Cluster${cluster_a}_mem${buffer_size_mb}MB_L1base${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                        if [ ! -d "$db_dir" ]; then
                            mkdir -p "$db_dir"
                        fi
                        ...
                        ../../../rocksdb/release/db_bench \
                            ...
                            --write_thread_max_yield_usec=50 \
                            --write_thread_slow_yield_usec=1 \
                            --db_write_buffer_size=0 \
                            --table_cache_numshardbits=12 \
                            --file_opening_threads=64 \
