...
cluster_value_size_map[48]=1000
...
for i in 40 35 13 51 49; do
    dir1="AutumnDB_SATASSD_MultiTwitterClusters_Space_Performance_pmem1"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            for cluster_a in "$i"; do  # 
                ...
                for workload_kvs in 100000000; do 
                    ...
                    for table_cache_size in ${cluster_table_size_map[$cluster_a]}; do
                        ...
                        log_file="AutumnDB_${num_format2}_val${value_size_twitter}_mem${buffer_size_mb}MB_Cluster${cluster_a}_CT0${ct0}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}.log"
                        data_file="/mnt/nvm/second_cluster${cluster_a}.sort" # 构建数据文件路径
                        data_file="/mnt/workloads/second_cluster${cluster_a}.sort"
                        memory_log_file="$(pwd)/AutumnDB_Memory_${num_format2}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}MiB_CT0${ct0}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}.log"      
                        ...
                        # 创建相应的目录
                        db_dir="/mnt/pmem1/Autumn10B/Load_Cluster${cluster_a}_mem${buffer_size_mb}MB_CT${ct0}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                        ...
                        ../../../rocksdb_autumn/release/db_bench \
                            ...
                            # perf stat -p $DB_BENCH_PID 2>&1 | tee "perf_PreLoad_stat_${num_format2}_val_${value_size_twitter}_Cluster${cluster_a}_mem${MEM}MiB_CT0${ct0}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}.txt" &
                            # PERF_PID=$!
