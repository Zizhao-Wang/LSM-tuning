...
for i in 49; do
    dir1="AutumnDB_PM_Benchmarking_ZippyDB_PM100M_test"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            for cluster_a in "$i"; do  # 
                ...
                for workload_kvs in 100000000 ; do 
                    ...
                    for table_cache_size in 0; do
                        ...
                        # 创建相应的目录
                        db_dir="/mnt/pmem0/Autumn10B/PreLoad_ZippyDB_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                        if [ ! -d "$db_dir" ]; then
                            mkdir -p "$db_dir"
                        fi
