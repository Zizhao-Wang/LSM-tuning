...
for i in 12; do
    dir1="AutumnDB_PM1_ClusterRanges_Benchmarking_Performance_with_delay"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            for cluster_a in "$i"; do  # 
                ...
                for workload_kvs in 100000000 ; do 
                    ...
                    for delay_time in  50 100 150 200; do
                        ...
                        log_file="AutumnDB_${num_format2}_val${value_size_twitter}_mem${buffer_size_mb}MB_Cluster${cluster_a}_CT0${ct0}_L1${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_range${range_length}F${f}delay${delay_time}.log"
