...
for i in {10..10}; do
    base_num=$(($billion * $i))
    dir1="${i}B_leveldb_uniform_tuning_structure(f_c0)_experiments_C0${level1base}"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
        for value_size in 128; do
            ...
            for buffer_size in 67108864; do
                for F in 10; do
                    ...
                    log_file="leveldb_${num_format}_val_${value_size}_mem${buffer_size_mb}MB_unifrom_factor${F}_L1base${level1base}MiB_L0CT${L0CT}_L0SDWT${L0SDWT}_L0SWT${L0SWT}.log"
                    data_file="/mnt/workloads/uniform_read_keys16_value128_1B.csv" # 构建数据文件路径
                    memory_log_file="/home/jeff-wang/LSM-tuning/comparedDBs/performance_test_scripts/leveldb_experiment/10B_leveldb_tuning_structure(f_c0)_experiments_C0${level1base}/leveldb_uniform_f${F}_memory_usage_${num_format}_key16_val${value_size}_mem${buffer_size_mb}MiB_factor${F}_L1base${level1base}_L0CT${L0CT}_L0SDWT${L0SDWT}_L0SWT${L0SWT}.log"  
                    ...
                    # 创建相应的目录
                    db_dir="/mnt/db_test2/LSM-tuning/level10/C0_${level1base}/leveldb_uniform_f${F}_mem${buffer_size_mb}MiB_L1base${level1base}_L0CT${L0CT}_L0SDWT${L0SDWT}_L0SWT${L0SWT}.log"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
                    ...
                    iostat -d 100 -x $DEVICE_NAME > leveldbf${F}_${num_format}_val_${value_size}_mem${buffer_size_mb}MB_uniform_IOstats_factor${F}_L1base${level1base}_L0CT${L0CT}_L0SDWT${L0SDWT}_L0SWT${L0SWT}.log &
                    PID_IOSTAT=$!
                            
                    cgexec -g memory:group32 ../../../leveldb/build_release_f_${F}_C0_${level1base}/db_bench \
                        --db=$db_dir \
                        --num=$num_entries \
                        ...
                        perf stat -p $DB_BENCH_PID 2>&1 | tee "perf_stat_${num_format}_val_${value_size}_mem${buffer_size_mb}MB_uniform_factor${F}_L1base${level1base}_L0CT${L0CT}_L0SDWT${L0SDWT}_L0SWT${L0SWT}.txt" &
                        PERF_PID=$!
