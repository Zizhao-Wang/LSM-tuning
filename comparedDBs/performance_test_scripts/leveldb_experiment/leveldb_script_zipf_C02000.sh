...
for i in {10..10}; do
    ...
    if [ ! -d "$dir1" ]; then
        ...
        for value_size in 128; do
            ...
            for zipf_a in 1.1 1.2 1.3 1.4 1.5; do  #  1.1  1.2 
                for buffer_size in 67108864; do
                    ...
                    # 创建相应的目录
                    db_dir="/mnt/db_test/level10B/leveldb_augmentation/leveldb_f${F}_${zipf_a}_mem${buffer_size_mb}MiB_level1base_${level1base}"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
                    ...
                    cgexec -g memory:group16 ../../../leveldb/build_release_f_10_C0_2000/db_bench \
                        --db=$db_dir \
                        --num=$num_entries \
