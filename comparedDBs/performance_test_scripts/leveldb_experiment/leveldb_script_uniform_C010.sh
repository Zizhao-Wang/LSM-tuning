        ...
        for value_size in 128; do
            ...
            for buffer_size in 67108864; do
                for F in 4 8 10 16 32; do
                    ...
                    iostat -d 100 -x $DEVICE_NAME > leveldbf${F}_${num_format}_val_${value_size}_mem${buffer_size_mb}MB_uniform_IOstats_factor${F}_level1base${level1base}MiB.log &
                    PID_IOSTAT=$!
                            
                    cgexec -g memory:group16 ../../../leveldb/build_release_f_${F}_C0_${level1base}/db_bench \
                        --db=$db_dir \
                        --num=$num_entries \
                        ...
                        perf stat -p $DB_BENCH_PID 2>&1 | tee "perf_stat_${num_format}_val_${value_size}_mem${buffer_size_mb}MB_uniform_factor${F}_level1base${level1base}MiB.txt" &
                        PERF_PID=$!
