            ...
            for cluster_a in "$i"; do  # 
                ...
                for workload_kvs in 100000000 ; do 
                    ...
                    for table_cache_size in 0 ; do
                        ...
                        log_file="RocksDB_ZippyDB_mem${buffer_size_mb}MB_CT0${ct0}_L1${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_iteration3.log"
                        data_file="/mnt/nvm/second_cluster${cluster_a}.sort" # 构建数据文件路径
                        memory_log_file="$(pwd)/RocksDB_Memory_ZippyDB_key${key_size_twitter}_val${value_size_twitter}_mem${buffer_size_mb}MiB_CT0${ct0}_L1${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}3.log"      
                        ...
                        # 创建相应的目录
                        db_dir="/mntdisk/rocks10B/PreLoad_Cluster${cluster_a}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                        if [ ! -d "$db_dir" ]; then
                            mkdir -p "$db_dir"
                        fi
