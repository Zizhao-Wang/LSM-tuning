...
cluster_block_cache_size_map[51]=0
...
key_sizes=""
value_sizes=""
l0_compaction_triggers=""
l0_slowdown_triggers=""
level_bases=""
level_multipliers=""

for workload_id in 35 13 40 49 51; do
    ...
    # level_bases values are in MiB, will be converted to bytes below
    case $workload_id in
        51) cache_sizes+="3145728";    open_files_list+="100"
            key_sizes+="16";           value_sizes+="1000"
            l0_compaction_triggers+="1"; l0_slowdown_triggers+="20"
            level_bases+="18000";        level_multipliers+="2" ;;
        49) cache_sizes+="8388608";    open_files_list+="100"
            key_sizes+="24";           value_sizes+="1000"
            l0_compaction_triggers+="1"; l0_slowdown_triggers+="12"
            level_bases+="17000";        level_multipliers+="2" ;;
        40) cache_sizes+="8388608";    open_files_list+="100"
            key_sizes+="24";           value_sizes+="1000"
            l0_compaction_triggers+="2"; l0_slowdown_triggers+="36"
            level_bases+="16000";        level_multipliers+="2" ;;
        13) cache_sizes+="8388608";    open_files_list+="500"
            key_sizes+="24";           value_sizes+="4266"
            l0_compaction_triggers+="80"; l0_slowdown_triggers+="160"
            level_bases+="4900";        level_multipliers+="8" ;;
        35) cache_sizes+="8388608";    open_files_list+="30"
            key_sizes+="24";           value_sizes+="1000"
            l0_compaction_triggers+="1"; l0_slowdown_triggers+="8"
            level_bases+="4000";        level_multipliers+="12" ;;
    esac
done

# Convert level_bases from MiB to bytes
level_bases_bytes=""
IFS=',' read -ra _lb_arr <<< "$level_bases"
for _lb in "${_lb_arr[@]}"; do
    if [ -n "$level_bases_bytes" ]; then
        level_bases_bytes+=","
    fi
    level_bases_bytes+="$((_lb * 1048576))"
done

dir1="RocksDB_MultiTwitterClusters_Benchmarking_Performance_NVM"
if [ ! -d "$dir1" ]; then
    mkdir $dir1
fi
        cd $dir1
                ...
                for workload_kvs in 100000000 ; do 
                    ...
                    for mu in 8; do
                        ...
                        log_file="SATune_${num_format2}_val${value_size_twitter}_mem${buffer_size_mb}MB_Cluster${cluster_a}_CT0${ct0}_L1${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_f${mu}.log"
                        ...
                        # 创建相应的目录
                        db_dir="/mnt/nvm/rocks10B/PreLoad_Cluster${cluster_a}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                        if [ ! -d "$db_dir" ]; then
                            mkdir -p "$db_dir"
                        fi
                        ...
                        ../../../../rocksdb/release/db_bench \
                            ...
                            --level0_compaction_trigger_per_workload="$l0_compaction_triggers" \
                            --level0_slowdown_trigger_per_workload="$l0_slowdown_triggers" \
                            --level_base_per_workload="$level_bases_bytes" \
                            --level_multiplier_per_workload="$level_multipliers" \
                            --compression_ratio=0 \
