...
for i in 35 40 51 13 49; do # 49 48 12
    dir1="RocksDB_Tiered_MultiTwitterClusters_Space_Performance_PM"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            for cluster_a in "$i"; do  # 
                ...
                for workload_kvs in 100000000 ; do 
                    ...
                    for table_cache_size in ${cluster_table_size_map[$cluster_a]}; do
                        ...
                        # 创建相应的目录
                        db_dir="/mnt/pmem1/rockstiered10B/PreLoad_Cluster${cluster_a}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                        if [ ! -d "$db_dir" ]; then
                            mkdir -p "$db_dir"
                        fi
