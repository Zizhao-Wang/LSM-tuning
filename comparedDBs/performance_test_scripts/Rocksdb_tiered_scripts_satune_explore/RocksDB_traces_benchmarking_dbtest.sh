...
for i in 51 49; do
    dir1="RocksDB_Tiered_MultiTwitterClusters_Benchmarking_Performance_db_test"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            for cluster_a in "$i"; do  # 
                ...
                for workload_kvs in 100000000 ; do 
                    ...
                    for table_cache_size in 0 ${cluster_table_size_map[$cluster_a]}; do
                        ...
                        # 创建相应的目录
                        db_dir="/mnt/db_test/rockstiered10B/PreLoad_Cluster${cluster_a}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                        if [ ! -d "$db_dir" ]; then
                            mkdir -p "$db_dir"
                        fi
