...
for i in 51; do
    dir1="RocksDB_Tiered_IntelS4510_ClusterRanges_Benchmarking_Performance"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            for cluster_a in "$i"; do  # 
                ...
                for workload_kvs in 100000000 ; do 
                    ...
                    for range_length in 10 50 1000 100; do
                        ...
                        # 创建相应的目录
                        db_dir="/mnt/db_test2/rockstiered10B/PreLoad_Cluster${cluster_a}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_range${range_length}"
                        if [ ! -d "$db_dir" ]; then
                            mkdir -p "$db_dir"
                        fi
