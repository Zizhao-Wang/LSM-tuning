...
for i in {10..10}; do
    base_num=$(($billion * $i))
    dir1="${i}B_RocksDB_SATASSD_TwitterCluster35_Benchmarking"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            ...
            for cluster_a in 35 ; do  # 
                ...
                num_format3=$(convert_to_billion_format "$num_kvs2")
                    ...
                    for table_cache_size in 300 1000 5000 ; do
                        ...
                        # 创建相应的目录
                        db_dir="/mnt/workloads/rocks10B/PreLoad_Cluster${cluster_a}_${num_format3}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_targetbase${target_file_base_mb}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
