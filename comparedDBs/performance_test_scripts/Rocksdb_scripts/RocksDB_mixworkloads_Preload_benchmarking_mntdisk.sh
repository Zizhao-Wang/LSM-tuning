...
for i in {10..10}; do
    base_num=$(($billion * $i))
    dir1="${i}B_RocksDB_Twitter_Cluster35_PreLoad_PerformanceSATASSD"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
        for value_size in 128; do
            ...
            for cluster_a in 35; do  # 
                ...
                for read_ratio in "${READ_RATIOS[@]}"; do
                    ...
                    log_file="RocksDB_MixWorkload_${num_format}_val${value_size_twitter}_mem${buffer_size_mb}MB_Cluster${cluster_a}_CT0${ct0}_L1${mb}_read_${read_ratio}_write_${write_ratio}.log"
                    data_file="/mnt/nvm/second_cluster${cluster_a}.sort" # 构建数据文件路径
                    memory_log_file="$(pwd)/RocksDB_PreLoad_${num_format}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}MiB_CT0${ct0}.log"      
                    ...
                    ../../../rocksdb/release/db_bench \
                        ...
                        --batch_size=1 \
                        --benchmarks=fillmixworkload,stats \
