...
# 排序字段：按照该字段升序排列
SORT_KEY = "average_latency_micros"
...
def load_rows(csv_path):
    ...
    with open(csv_path, newline='') as f:
        ...
        for r in reader:
            # 转换数值列
            for num_col in ["average_latency_micros",
...
def main():
    ...
    for idx, r in enumerate(results):
        values = [f"{r.get(col):.6f}" if isinstance(r.get(col), float) else str(r.get(col)) for col in METRICS]
