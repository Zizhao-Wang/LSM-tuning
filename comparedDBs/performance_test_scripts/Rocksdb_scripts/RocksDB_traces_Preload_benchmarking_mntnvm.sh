...
for i in {10..10}; do
    base_num=$(($billion * $i))
    dir1="${i}B_RocksDB_NVMe_Twitter35_PreLoad_Performance"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
        for value_size in 128; do
            ...
            for cluster_a in 35; do  # 
                ...
                for table_cache_size in 1000; do
                    ...
                    # 创建相应的目录
                    db_dir="/mnt/nvm/rocks10B/PreLoad_Cluster${cluster_a}_${num_format}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_targetbase${target_file_base_mb}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
