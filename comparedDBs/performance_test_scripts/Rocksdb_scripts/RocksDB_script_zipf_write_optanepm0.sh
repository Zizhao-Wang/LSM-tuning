...
for i in {10..10}; do
    base_num=$(($billion * $i))
    dir1="${i}B_RocksDB_Write_Performance_OptanePM"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
        for value_size in 128; do
            ...
            for zipf_a in 1.1 1.2 1.3 1.4 1.5; do  # 
                ...
                for buffer_size in 2097152 8388608 33554432; do
                    ...
                    # 创建相应的目录
                    db_dir="/mnt/pmem0/rocks10B/mem${buffer_size_mb}MB_zipf${zipf_a}_CT${ct0}_L1base${mb}_targetbase${target_file_base_mb}"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
