        ...
        for value_size in 128; do
            ...
            for cluster_a in 51; do  # 
                ...
                for table_cache_size in 300 1000 5000 10000; do
                    ...
                    memory_log_file="$(pwd)/RocksDB_PreLoadMemeory_${num_format}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}MiB_CT0${ct0}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}.log"      
                    ...
                    # 创建相应的目录
                    db_dir="/mnt/db_test/rocks10B/PreLoad_Cluster${cluster_a}_${num_format}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_targetbase${target_file_base_mb}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
