...
# 用途：为 LevelDB 生成 C0=100 的构建配置，并记录构建日志
...
echo "Build Configuration: C0=100, CMAKE_BUILD_TYPE=Debug" >> "$LOG_FILE"
echo "Start Time: $(date '+%Y-%m-%d %H:%M:%S')" >> "$LOG_FILE"
echo "-----------------------------------------" >> "$LOG_FILE"

# 进入构建目录，配置并编译
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Configuring and building for C0=100"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Build succeeded for C0=100" | tee -a "$LOG_FILE"
