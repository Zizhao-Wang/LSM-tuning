...
# 用途：为 LevelDB 生成 C0=500 的构建配置，并记录构建日志

# 获取脚本所在目录的绝对路径
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# 定义构建目录的绝对路径
BUILD_DIR="${SCRIPT_DIR}/../leveldb/build_release_f_10_C0_500"
...
echo "Build Configuration: C0=500, CMAKE_BUILD_TYPE=Debug" >> "$LOG_FILE"
echo "Start Time: $(date '+%Y-%m-%d %H:%M:%S')" >> "$LOG_FILE"
echo "-----------------------------------------" >> "$LOG_FILE"

# 进入构建目录，配置并编译
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Configuring and building for C0=500"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Build succeeded for C0=500" | tee -a "$LOG_FILE"
