...
for i in 12; do # 40 49 35 51 30 1
    dir1="10B_Smoose_Clustersrange_Benchmarking_Performance_pmem1_with_dealy22"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
    cd $dir1
    stats_interva=10000000
        for cluster_a in "$i"; do  # 
                ...
                for delay_time in 50 100 150 200; do
                    ...
                    log_file="Smoose_PreLoad_${num_format}_key${key_size_twitter}_val${value_size_twitter}_mem${buffer_size_mb}MB_Cluster${cluster_a}_CT0${ct0}_L1${mb}_read_${read_ratio}_write_${write_ratio}Table${table_cache_size}BlockCache${blk_cache_size}Perf${perf_tier}Range${range_length}delay${delay_time}.log"
                    ...
                    memory_log_file="$(pwd)/Smoose_PreLoadMemory_${num_format}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}MiB_CT0${ct0}Range${range_length}delay${delay_time}.log"      
