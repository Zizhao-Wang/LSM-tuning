...
WORKLOAD_SEQUENCE=(35 13 40 49 51)
...
cluster_table_size_map[51]=100;   cluster_table_size_map[49]=100;   cluster_table_size_map[40]=100
cluster_table_size_map[13]=500; cluster_table_size_map[35]=30;   

dir1="Smoose_DynamicWorkloads_Benchmarking_pmem1_reverse"
if [ ! -d "$dir1" ]; then
    mkdir "$dir1"
fi
cd "$dir1"
...
for workload_kvs in 100000000; do
    ...
    for perf_tier in 1; do
        ...
        db_dir="/mnt/pmem1/MooseLSM/Dynamic_WL${workload_tag}_${num_format}_mem${buffer_size_mb}MB_CT${ct0}"
        if [ ! -d "$db_dir" ]; then
            mkdir -p "$db_dir"
        fi
        if [ "$(ls -A $db_dir)" ]; then
