...
for i in 12; do # 40 49 35 51 30 1
    dir1="10B_Smoose_SATASSD_MultiTwitterClusters_Benchmarking_Performance_mntdisk_WithStdDev"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
    cd $dir1
    stats_interva=1000000
        for cluster_a in "$i"; do  # 
                ...
                for perf_tier in 1 ; do
                    ...
                    # 创建相应的目录
                    db_dir="/mntdisk/MooseLSM/PreLoad_Cluster${cluster_a}_${num_format}_key${key_size_twitter}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_mixed_read_${read_ratio}_write_${write_ratio}"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
