...
for i in 12; do # 40 49 35 51 30 1
    dir1="10B_Smoose_Clustersrange_Benchmarking_Performance_pmem11"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
    cd $dir1
    stats_interva=10000000
        for cluster_a in "$i"; do  # 
                ...
                for range_length in 10; do
                    ...
                    log_file="Smoose_PreLoad_${num_format}_key${key_size_twitter}_val${value_size_twitter}_mem${buffer_size_mb}MB_Cluster${cluster_a}_CT0${ct0}_L1${mb}_read_${read_ratio}_write_${write_ratio}Table${table_cache_size}BlockCache${blk_cache_size}Perf${perf_tier}Range${range_length}.log"
                    # data_file="/mnt/nvm/second_cluster${cluster_a}.sort" # 构建数据文件路径
                    data_file="/mnt/workloads/second_cluster${cluster_a}.sort" # 构建数据文件路径
                    memory_log_file="$(pwd)/Smoose_PreLoadMemory_${num_format}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}MiB_CT0${ct0}Range${range_length}.log"      
                    ...
                    # 创建相应的目录
                    db_dir="/mnt/pmem1/MooseLSM/PreLoad_Cluster${cluster_a}_${num_format}_key${key_size_twitter}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_mixed_read_${read_ratio}_write_${write_ratio}"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
                    ...
                    # range_query_source:1 means put, 0 means get, the default is: 0
                    ../build/smoose_tester \
                        --workload=mixed \
                        --db=$db_dir \
