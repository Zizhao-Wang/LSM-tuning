...
//
rocksdb::Status PersistentCacheTier::Open() {
  if (next_tier_) {
...
rocksdb::Status PersistentCacheTier::Close() {
  if (next_tier_) {
...
bool PersistentCacheTier::Erase(const rocksdb::Slice& /*key*/) {
...
rocksdb::PersistentCache::StatsType PersistentCacheTier::Stats() {
  if (next_tier_) {
...
rocksdb::Status PersistentTieredCache::Open() {
  assert(!tiers_.empty());
  return tiers_.front()->Open();
}

rocksdb::Status PersistentTieredCache::Close() {
  assert(!tiers_.empty());
  rocksdb::Status status = tiers_.front()->Close();
  if (status.ok()) {
    tiers_.clear();
  }
  return status;
...
bool PersistentTieredCache::Erase(const rocksdb::Slice& key) {
  assert(!tiers_.empty());
  return tiers_.front()->Erase(key);
}

rocksdb::PersistentCache::StatsType PersistentTieredCache::Stats() {
  assert(!tiers_.empty());
...
rocksdb::Status PersistentTieredCache::Insert(const rocksdb::Slice& page_key, const char* data,
                                     const size_t size) {
  assert(!tiers_.empty());
  return tiers_.front()->Insert(page_key, data, size);
}

rocksdb::Status PersistentTieredCache::Lookup(const rocksdb::Slice& page_key,
