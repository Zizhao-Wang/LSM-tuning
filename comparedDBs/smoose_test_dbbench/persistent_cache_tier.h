...
struct PersistentCacheConfig {
  explicit PersistentCacheConfig(
      rocksdb::Env* const _env, const std::string& _path, const uint64_t _cache_size,
      const std::shared_ptr<rocksdb::Logger>& _log,
  ...
  //
  rocksdb::Status ValidateSettings() const {
    // (1) check pre-conditions for variables
    if (!env || path.empty()) {
      return rocksdb::Status::InvalidArgument("empty or null args");
    }
    ...
    if (cache_size < cache_file_size || write_buffer_size >= cache_file_size ||
      ...
      return rocksdb::Status::InvalidArgument("invalid cache size");
    }
    ...
    if (!writer_qdepth || writer_dispatch_size > write_buffer_size ||
      ...
      return rocksdb::Status::InvalidArgument("invalid writer settings");
    }
  ...
  PersistentCacheConfig MakePersistentCacheConfig(
      ...
      const std::shared_ptr<rocksdb::Logger>& log);
...
class PersistentCacheTier : public rocksdb::PersistentCache {
 public:
  ...
  virtual rocksdb::Status Insert(const rocksdb::Slice& page_key, const char* data,
                        const size_t size) override = 0;

  // Lookup page cache by page identifier
  virtual rocksdb::Status Lookup(const rocksdb::Slice& page_key, std::unique_ptr<char[]>* data,
...
class PersistentTieredCache : public PersistentCacheTier {
 public:
  ...
  rocksdb::Status Open() override;
  rocksdb::Status Close() override;
  bool Erase(const rocksdb::Slice& key) override;
  std::string PrintStats() override;
  PersistentCache::StatsType Stats() override;
  rocksdb::Status Insert(const rocksdb::Slice& page_key, const char* data,
                const size_t size) override;
  rocksdb::Status Lookup(const rocksdb::Slice& page_key, std::unique_ptr<char[]>* data,
