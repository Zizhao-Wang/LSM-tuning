...
for i in 40 ; do
    dir1="SATune_SATASSD_TwitterCluster${i}_Load_Separation"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            ...
            for cluster_a in "$i"; do  # 
                for ct0 in 4 ; do  # 
                for mb in 100; do
                for multiplier in 5; do
                    echo "================================================="
                    echo "Running benchmark with multiplier = $multiplier"
                    echo "================================================="
                for multiplier_switch in 3; do
                ...
                for table_cache_size in 1000 ; do
                    ...
                    db_dir="/mnt/db_test/SATune10B/PreLoad_Cluster${cluster_a}_${num_format}_mem${buffer_size_mb}MB_CT${ct0}_L1${mb}_targetbase${target_file_base_mb}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}"
                    db_dir_other_levels="/mnt/workloads/SATune10B/PreLoad_Cluster${cluster_a}_${num_format}_mem${buffer_size_mb}MB_CT${ct0}_L1${mb}_targetbase${target_file_base_mb}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}"
                    level_config_tag="L1-6_on_1.92TDisk"
                    level_paths_arg="1-6:${db_dir_other_levels}"

                    log_file="SATune_PreLoad_${num_format}_val${value_size_twitter}_mem${buffer_size_mb}M_Cluster${cluster_a}_CT0${ct0}_L1${mb}_targetbase${target_file_base_mb}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}_${level_config_tag}.log"

                    data_file="/mnt/nvm/second_cluster${cluster_a}.sort" # 构建数据文件路径
                    memory_log_file="$(pwd)/SATune_PreLoad_${num_format}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}M_L1${mb}_CT0${ct0}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}.log"
                    compaction_log_file="$(pwd)/Compaction_${num_format}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}M_L1${mb}_CT0${ct0}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}_${level_config_tag}.log"  
                    ...
                    if [ ! -d "$db_dir_other_levels" ]; then
                        mkdir -p "$db_dir_other_levels"
                    fi

                    # 检查目录是否为空，如果不为空则删除所有内容
                    if [ "$(ls -A $db_dir_other_levels)" ]; then
                        rm -rf "${db_dir_other_levels:?}/"*
                    fi
                    ...
                    ../../SATune/release/db_bench \
                        ...
                        perf stat -p $DB_BENCH_PID 2>&1 | tee "perf_PreLoad_stat_${num_format}_val_${value_size_twitter}_Cluster${cluster_a}_mem${MEM}M_L1${mb}_CT0${ct0}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}.txt" &
                        PERF_PID=$!
