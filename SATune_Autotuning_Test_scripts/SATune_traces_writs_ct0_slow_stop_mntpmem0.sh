...
cluster_num_kvs_map[13]="2000000"
...
for i in 12; do
    dir1="SATune_PM_TwitterCluster${i}_Benchmarking_Performance_switch10"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            ...
            for cluster_a in "$i"; do  # 
                ...
                for multiplier in 10; do
                for after_multiplier in 50; do    
                for multiplier_switch in 10; do
                    echo "================================================="
                ...
                for table_cache_size in 1000 ; do
                    ...
                    log_file="SATune_PreLoad_${num_format}_val${value_size_twitter}_mem${buffer_size_mb}MB_Cluster${cluster_a}_CT0${ct0}_L1${mb}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}.log"
                    ...
                    # 创建相应的目录
                    db_dir="/mnt/pmem0/SATune10B/PreLoad_Cluster${cluster_a}_${num_format}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_targetbase${target_file_base_mb}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
                    ...
                    ../../SATune/release/db_bench \
                        ...
                        perf stat -p $DB_BENCH_PID 2>&1 | tee "perf_PreLoad_stat_${num_format}_val_${value_size_twitter}_Cluster${cluster_a}_mem${MEM}MiB_CT0${ct0}_F${multiplier}.txt" &
                        PERF_PID=$!
