        ...
        cd $dir1
            ...
            for cluster_a in "$i"; do  # 
                ...
                for table_cache_size in 1000 ; do
                    ...
                    log_file="SATune_PreLoad_${num_format}_val${value_size_twitter}_mem${buffer_size_mb}M_Cluster${cluster_a}_CT0${ct0}_L1${mb}_targetbase${target_file_base_mb}_F${multiplier}.log"
                    data_file="/mnt/nvm/second_cluster${cluster_a}.sort" # 构建数据文件路径
                    memory_log_file="$(pwd)/SATune_PreLoad_${num_format}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}M_L1${mb}_CT0${ct0}_F${multiplier}.log"
                    compaction_log_file="$(pwd)/Compaction_${num_format}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}M_L1${mb}_CT0${ct0}_F${multiplier}.log"  
                    ...
                    # 创建相应的目录
                    db_dir="/mnt/db_test/SATune10B/PreLoad_Cluster${cluster_a}_${num_format}_mem${buffer_size_mb}MB_CT${ct0}_L1${mb}_targetbase${target_file_base_mb}_F${multiplier}"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
                    ...
                    ../../SATune/release/db_bench \
                        ...
                        --multiplier_switch_level=8 \
                        --max_bytes_for_level_multiplier_after_switch=$multiplier \
                        ...
                        perf stat -p $DB_BENCH_PID 2>&1 | tee "perf_PreLoad_stat_${num_format}_val_${value_size_twitter}_Cluster${cluster_a}_mem${MEM}M_L1${mb}_CT0${ct0}_F${multiplier}.txt" &
                        PERF_PID=$!
