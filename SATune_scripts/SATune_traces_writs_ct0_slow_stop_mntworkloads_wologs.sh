        ...
        cd $dir1
            ...
            for cluster_a in "$i"; do  # 
                ...
                for table_cache_size in 1000 ; do
                    ...
                    # 创建相应的目录
                    db_dir="/mnt/workloads/SATune10B/PreLoad_Cluster${cluster_a}_${num_format}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_targetbase${target_file_base_mb}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_F${multiplier}_2F${after_multiplier}"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
