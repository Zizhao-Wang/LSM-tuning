...
for i in {10..10}; do
    base_num=$(($billion * $i))
    dir1="${i}B_SATune_TwitterCluster${run_cluster}_Benchmarking"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
        for value_size in 128; do
            ...
            for cluster_a in $run_cluster; do  # 
                for ct0 in 4 ; do  # 
                for mb in 512; do
                ...
                num_format3=$(convert_to_billion_format "$num_kvs2")
                    ...
                    for table_cache_size in 300 1000 5000 10000; do
                        ...
                        log_file="SATune_${num_format2}_in${num_format3}_val${value_size_twitter}_mem${buffer_size_mb}MB_Cluster${cluster_a}_CT0${ct0}_level1base${mb}_targetbase${target_file_base_mb}_Blk${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}.log"
                        data_file="/mnt/nvm/second_cluster${cluster_a}.sort" # 构建数据文件路径
                        memory_log_file="$(pwd)/SATune_BenchMemory_${num_format2}in${num_format3}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}MiB_CT0${ct0}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}.log"      
