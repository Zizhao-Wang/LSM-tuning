...
for i in 40 ; do
    dir1="SATune_SATASSD_TwitterCluster${i}_LoadPerformance_PM"
    if [ ! -d "$dir1" ]; then
        mkdir $dir1
    fi
        cd $dir1
            ...
            for cluster_a in "$i"; do  # 
                for ct0 in 4 ; do  # 
                for mb in 100 ; do
                for multiplier in 5 10; do
                for multiplier_switch in 8; do
                for after_multiplier in 20 30 50; do
                    echo "================================================="
                ...
                for table_cache_size in 1000 ; do
                    ...
                    # buffer_size=2097152
                    # if [[ $multiplier_switch -ge 8 ]]; then
                    #     continue
                    # fi
                    ...
                    log_file="SATune_PreLoad_${num_format}_val${value_size_twitter}_mem${buffer_size_mb}MB_Cluster${cluster_a}_CT0${ct0}_L1${mb}_targetbase${target_file_base_mb}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}.log"
                    data_file="/mnt/nvm/second_cluster${cluster_a}.sort" # 构建数据文件路径
                    memory_log_file="$(pwd)/SATune_PreLoad_${num_format}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}MiB_CT0${ct0}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}.log"
                    compaction_log_file="$(pwd)/Compaction_${num_format}_key${key_size_twitter}_val${value_size_twitter}_Cluster${cluster_a}_mem${buffer_size_mb}MiB_CT0${ct0}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}.log"  
                    ...
                    if [ -f "compaction_log_file" ]; then
                        rm  "compaction_log_file"
                    fi

                    # 创建相应的目录
                    db_dir="/mnt/pmem0/SATune10B/PreLoad_Cluster${cluster_a}_${num_format}_mem${buffer_size_mb}MB_CT${ct0}_L1base${mb}_targetbase${target_file_base_mb}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}"
                    if [ ! -d "$db_dir" ]; then
                        mkdir -p "$db_dir"
                    fi
                    ...
                    ../../SATune/release/db_bench \
                        --db=$db_dir \
                        --level_paths="0:/ssd_path/level0,4:/hdd_path/level4" \
                        ...
                        perf stat -p $DB_BENCH_PID 2>&1 | tee "perf_PreLoad_stat_${num_format}_val_${value_size_twitter}_Cluster${cluster_a}_mem${MEM}MiB_CT0${ct0}_Block${blk_size}_Blkcache${blk_cache_size}_Tabcache${table_cache_size}_F${multiplier}_Switch${multiplier_switch}_2F${after_multiplier}.txt" &
                        PERF_PID=$!
