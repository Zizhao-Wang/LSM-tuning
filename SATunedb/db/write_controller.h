...
class WriteController {
 public:
  explicit WriteController(uint64_t _delayed_write_rate = 1024u * 1024u * 32u,
      ...
      : total_stopped_(0),
        ...
        credit_in_bytes_(0),
        next_refill_time_(0),
        low_pri_rate_limiter_(NewGenericRateLimiter(low_pri_rate_bytes_per_sec)),
        current_stall_cause_(WriteStallCause::kNone),
        current_l0_file_count_(0){
  ...
  void SetStallCause(WriteStallCause cause) {
    current_stall_cause_.store(cause, std::memory_order_relaxed);
  }
  
  // ★ 新增：获取当前 stall 原因
  WriteStallCause GetStallCause() const {
    return current_stall_cause_.load(std::memory_order_relaxed);
  }

  // SATune: 设置当前 L0 文件数
  void SetL0FileCount(int count) {
    current_l0_file_count_.store(count, std::memory_order_relaxed);
  }
  
  // SATune: 获取当前 L0 文件数
  int GetL0FileCount() const {
    return current_l0_file_count_.load(std::memory_order_relaxed);
  }

  // ★ 新增：检查是否是 L0 导致的 stall
  bool IsL0Stall() const {
    WriteStallCause cause = current_stall_cause_.load(std::memory_order_relaxed);
    return cause == WriteStallCause::kL0FileCountLimit || cause == WriteStallCause::kPendingCompactionBytes;
  }
 ...
 private:
  ...
  // ★ 新增：当前 stall 原因
  std::atomic<WriteStallCause> current_stall_cause_;
  std::atomic<int> current_l0_file_count_;
};
