...
class ColumnFamilyData {
 public:
  ...
  // ============================================================================
  // SATune: Per-level write stall statistics for performance tuning
  // Track stall time and count for each LSM level to correlate with read performance
  // ============================================================================
  struct LevelStallStats {
    std::atomic<uint64_t> total_slowdown_micros{0};  // SATune: Total slowdown time in microseconds
    std::atomic<uint64_t> total_slowdown_count{0};   // SATune: Number of slowdown events
    std::atomic<uint64_t> total_stop_micros{0};      // SATune: Total stop time in microseconds
    std::atomic<uint64_t> total_stop_count{0};       // SATune: Number of stop events

    //=====SATune: Maximum L0 files exceeded trigger count in this batch
    //=====Example: if trigger=4 and current L0 files=8, then exceed=4
    std::atomic<int> max_l0_exceed_trigger{0};

    // ========== Batch Statistics (reset each batch) ==========
    std::atomic<uint64_t> batch_slowdown_micros{0};
    std::atomic<uint64_t> batch_slowdown_count{0};
    std::atomic<uint64_t> batch_stop_micros{0};
    std::atomic<uint64_t> batch_stop_count{0};

    // ========== Batch Statistics (reset each batch) ==========
    std::atomic<uint64_t> normal_tuning_slowstop_micros{0};

    // ========== Batch Statistics (reset each batch) ==========
    std::atomic<uint64_t> LG_tuning_slowstop_micros{0};

    std::atomic<int> batch_max_l0_exceed_trigger{0};

    uint64_t GetBatchTotalStallMicros() const {
      return batch_stop_micros.load(std::memory_order_relaxed) +
        batch_slowdown_micros.load(std::memory_order_relaxed);
    }

    uint64_t GetTotalStallMicros() const {
      return total_stop_micros.load(std::memory_order_relaxed) +
        total_slowdown_micros.load(std::memory_order_relaxed);
    }

    bool HasBatchStall() const {
      return GetBatchTotalStallMicros() > 0;
    }

    bool HasBatchOverflow() const {
      return batch_max_l0_exceed_trigger.load(std::memory_order_relaxed) > 0;
    }

    void ResetL0OverflowStallStats() {
      batch_slowdown_micros.store(0, std::memory_order_relaxed);
      batch_slowdown_count.store(0, std::memory_order_relaxed);
      batch_stop_micros.store(0, std::memory_order_relaxed);
      batch_stop_count.store(0, std::memory_order_relaxed);
      batch_max_l0_exceed_trigger.store(0, std::memory_order_relaxed);
    }

    void ResetNormalTuningStallStats() {
      normal_tuning_slowstop_micros.store(0, std::memory_order_relaxed);

    }

    void ResetLGTuningStallStats() {
      LG_tuning_slowstop_micros.store(0, std::memory_order_relaxed);
    }
  };
  ...
  // ============================================================================
  // SATune: Record a stall event for a specific level
  // @param level: LSM level (0-6)
  // @param micros: Duration of the stall in microseconds
  // @param is_stop: true for stop, false for slowdown
  // ============================================================================
  void RecordLevelStall(uint64_t micros, bool is_stop, int current_l0_files, int l0_trigger) {
    // Update cumulative stats
    if (is_stop) {
      level_stall_stats_.total_stop_micros.fetch_add(micros, std::memory_order_relaxed);
      level_stall_stats_.total_stop_count.fetch_add(1, std::memory_order_relaxed);
      level_stall_stats_.batch_stop_micros.fetch_add(micros, std::memory_order_relaxed);
      level_stall_stats_.batch_stop_count.fetch_add(1, std::memory_order_relaxed);

      level_stall_stats_.normal_tuning_slowstop_micros.fetch_add(micros, std::memory_order_relaxed);
      level_stall_stats_.LG_tuning_slowstop_micros.fetch_add(micros, std::memory_order_relaxed);
    } else {
      level_stall_stats_.total_slowdown_micros.fetch_add(micros, std::memory_order_relaxed);
      level_stall_stats_.total_slowdown_count.fetch_add(1, std::memory_order_relaxed);
      level_stall_stats_.batch_slowdown_micros.fetch_add(micros, std::memory_order_relaxed);
      level_stall_stats_.batch_slowdown_count.fetch_add(1, std::memory_order_relaxed);

      level_stall_stats_.normal_tuning_slowstop_micros.fetch_add(micros, std::memory_order_relaxed);
      level_stall_stats_.LG_tuning_slowstop_micros.fetch_add(micros, std::memory_order_relaxed);
    }
    
    // Update max overflow count (batch only)
    if (current_l0_files > l0_trigger) {
      int exceed_count = current_l0_files - l0_trigger;
      
      // CAS loop to update max value atomically
      int old_max = level_stall_stats_.batch_max_l0_exceed_trigger.load(std::memory_order_relaxed);
      while (exceed_count > old_max) {
        if (level_stall_stats_.batch_max_l0_exceed_trigger.compare_exchange_weak(old_max, exceed_count, std::memory_order_relaxed)) {
          break;
        }
      }
    }
  ...
  // ============================================================================
  // SATune: Get stall statistics for a specific level
  // ============================================================================
  const LevelStallStats& GetLevelStallStats() const {
    return level_stall_stats_;
  }

  LevelStallStats* GetMutableLevelStallStats() {
    return &level_stall_stats_;
  }

  // ============================================================================
  // SATune: Reset all level stall statistics (for testing/benchmarking)
  // ============================================================================
  void ResetTotalStallStats() {
    level_stall_stats_.total_slowdown_micros.store(0, std::memory_order_relaxed);
    level_stall_stats_.total_slowdown_count.store(0, std::memory_order_relaxed);
    level_stall_stats_.total_stop_micros.store(0, std::memory_order_relaxed);
    level_stall_stats_.total_stop_count.store(0, std::memory_order_relaxed);
  }
 ...
 private:
  ...
  // ============================================================================
  // SATune: Array of per-level stall statistics
  // Index corresponds to LSM level (0 = L0, 1 = L1, etc.)
  // ============================================================================
  LevelStallStats level_stall_stats_;
