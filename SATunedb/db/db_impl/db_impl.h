...
#include "db/tuner/workload_monitor.h"
#include "db/tuner/auto_tuner.h"
...
class DBImpl : public DB {
 public:
  ...
  //
  void ApplyTuning(ColumnFamilyData* cfd, const TuningResult& tuning);
  struct CompactionPrediction {
    bool needed = false;     // 是否需要 Compaction
    int input_level = -1;    // 谁是 Input
    int output_level = -1;   // 谁是 Output
    double score = 0.0;      // 分数是多少
  };
  CompactionPrediction GetNextCompactionInfo(ColumnFamilyData* cfd);
  ...
  struct GetImplOptions {
    ...
    int* source_level = nullptr;  // -1=memtable, 0-6=LSM levels, -2=not found
    int* number_of_operands = nullptr;
  };

  struct GetResult {
    Status status;
    int source_level;  // -1=memtable, 0-6=LSM levels
  };
  ...
  Status GetImpl(const ReadOptions& read_options,
                 ColumnFamilyHandle* column_family, const Slice& key,
                 PinnableSlice* value, std::string* timestamp, int* source_level=nullptr);
 ...
 protected:
  ...
  struct CompactionTuningState {
    std::atomic<bool> first_compaction_done;
    std::atomic<uint64_t> total_compactions; 
    
    CompactionTuningState() 
    :first_compaction_done(false),
      total_compactions(0) {}

    bool IsFirstCompaction() const {
      return !first_compaction_done.load(std::memory_order_relaxed);
    }
    
    void MarkFirstCompactionDone() {
      first_compaction_done.store(true, std::memory_order_relaxed);
    }
    ...
    uint64_t GetTotalCompactions() const {
      return total_compactions.load(std::memory_order_relaxed);
    }
  };


  std::shared_ptr<Logger> tuning_log_;
  std::atomic<uint64_t> satune_predict_total_micros_;
  PerformanceMonitor performance_monitor_;
  CompactionTuningState compaction_tuning_state_;
 ...
 private:
  ...
  struct PrepickedCompaction {
    ...
    // ✅ 新增：SATune 标志，指示是否应该跳过 compaction
    bool skip_due_to_tuning = false;
    int input_level_for_satune;
  };
  ...
  // ============================================================================
  // SATune: API to retrieve per-level L0 stall statistics
  // Used for correlating write stalls with read performance degradation
  // ============================================================================
  void GetL0StallStats(int level, 
                       uint64_t* slowdown_micros,
                       uint64_t* slowdown_count,
  ...
  Status BackgroundCompaction(bool* madeProgress, JobContext* job_context,
                              ...
                              PrepickedCompaction* prepicked_compaction,
                              Env::Priority thread_pri, bool skip_due_to_tuning);
