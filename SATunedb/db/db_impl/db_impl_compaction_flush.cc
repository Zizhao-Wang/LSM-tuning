...
// 获取指定 Column Family 下一次最可能的 Compaction 信息
DBImpl::CompactionPrediction DBImpl::GetNextCompactionInfo(ColumnFamilyData* cfd) {
  CompactionPrediction prediction;

  if (cfd == nullptr) {
    return prediction;
  }

  // 1. 获取 VersionStorageInfo
  // 注意：调用此函数时必须持有 mutex_，BackgroundCallCompaction 已经持有了，所以是安全的
  VersionStorageInfo* vstorage = cfd->current()->storage_info();
  if (vstorage == nullptr) {
    return prediction;
  }

  // 2. 检查是否有需要 Compaction 的层
  // RocksDB 已经把分数最高的放在 CompactionScore(0) 了，不用循环
  prediction.score = vstorage->CompactionScore(0);
  
  // 3. 如果最高分都 < 1.0，说明根本不需要 Compaction
  if (prediction.score < 1.0) {
    prediction.needed = false;
    return prediction;
  }

  // 4. 填充详细信息
  prediction.needed = true;
  prediction.input_level = vstorage->CompactionScoreLevel(0);

  // 5. 计算 Output Level
  // 逻辑：如果是 L0，去 base_level；否则去 input_level + 1
  if (prediction.input_level == 0) {
    prediction.output_level = vstorage->base_level();
  } else {
    prediction.output_level = prediction.input_level + 1;
  }
...
void DBImpl::BackgroundCallCompaction(PrepickedCompaction* prepicked_compaction,
                       ...
                       immutable_db_options_.info_log.get());
  {
    ...
    // fprintf(stderr, "[DEBUG] Starting compaction prediction section\n");       
    ColumnFamilyData* cfd = nullptr;
    if (versions_ && versions_->GetColumnFamilySet()) {
      cfd = versions_->GetColumnFamilySet()->GetDefault();
    }
    CompactionPrediction pred = GetNextCompactionInfo(cfd);  
    // fprintf(stderr, "[CompactionCheck] cfd=%p level=%d->%d score=%.2f\n", 
    //     (void*)cfd, pred.input_level, pred.output_level, pred.score); 
    TuningResult tuning_results;

    tuning_results = performance_monitor_.CheckAndHandleBatch(this,pred.input_level,cfd);
    bool should_skip = false;
    if(!compaction_tuning_state_.IsFirstCompaction()){
      if (tuning_results.needs_change){
        ApplyTuning(cfd,tuning_results);
        auto* vstorage = cfd->current()->storage_info();
        double new_score = vstorage->CompactionScore(pred.input_level);
        if (new_score < 1.0) {
          ROCKS_LOG_INFO(tuning_log_,"[SATune] Will skip compaction after tuning:L%d score %.2f < 1.0\n",
            pred.input_level, new_score);
          should_skip = true;
        }else{
          ROCKS_LOG_INFO(tuning_log_,"[SATune] Will not skip compaction even if some changes happen:L%d score %.2f!\n",
            pred.input_level, new_score);
        }
      }else{
        ROCKS_LOG_INFO(tuning_log_,"[SATune] This tuning does not change anything");
      }
      tuning_log_->Flush();
    }
    // 2. [计时结束]
  ...
  // 4. [累加到总时间]
  // fetch_add 返回的是加之前的值，所以如果你想打印当前的最新总值，需要加上 elapsed_micros
  uint64_t previous_total = satune_predict_total_micros_.fetch_add(elapsed_micros);
  uint64_t current_total = previous_total + elapsed_micros;
  // 5. [打印日志]
  double elapsed_sec = (double)elapsed_micros / 1000000.0;
  double total_sec = (double)current_total / 1000000.0;
  ROCKS_LOG_INFO(tuning_log_, "[SATune] Prediction Latency: %lld us (%.6f s), Total Time: %llu us (%.6f s)\n", 
    static_cast<long long>(elapsed_micros),elapsed_sec, static_cast<unsigned long long>(current_total), total_sec);

    Status s = BackgroundCompaction(&made_progress, &job_context, &log_buffer,
                                    prepicked_compaction, bg_thread_pri,should_skip);

    if (compaction_tuning_state_.IsFirstCompaction() && s.ok()) {
      compaction_tuning_state_.MarkFirstCompactionDone();
      compaction_tuning_state_.IncrementCompactionCount();  
      if (tuning_results.needs_change) {
        ApplyTuning(cfd, tuning_results);
        ROCKS_LOG_INFO(tuning_log_, "[SATune] Initial tuning applied: %s\n\n", tuning_results.reason.c_str());
      }
      tuning_log_->Flush();
    }
...
Status DBImpl::BackgroundCompaction(bool* made_progress,
                                    ...
                                    Env::Priority thread_pri,bool skip_due_to_tuning = false) {
  ...
  *made_progress = false;
  ...
  //====SATune
  int compaction_input_level = -1;
  bool marked_in_progress = false;
  //=======SATune
  ...
  if (is_manual) {
    ...
    if (skip_due_to_tuning) {
      ROCKS_LOG_INFO(tuning_log_, "[SATune] Skipping compaction due to tuning decision\n\n");
      return Status::OK();
    }
    ...
    if (!mutable_cf_options->disable_auto_compactions && !cfd->IsDropped()) {
      ...
      if (c != nullptr) {
        ...
        if (!enough_room) {
          ...
          //======SATune======
          compaction_input_level = c->start_level(); 
          performance_monitor_.MarkCompactionInProgress(compaction_input_level);
          marked_in_progress = true;
          int output_level = c->output_level();
          performance_monitor_.MarkCompactionReady(output_level);
          if(c->output_level()==2){
            ROCKS_LOG_INFO(tuning_log_,"level 2 is crteated!\n\n");
          }
          //======SATune======
        ...
        ROCKS_LOG_INFO(tuning_log_,"some conflicts happen and compaction can not be executed!\n\n");
      }
    }
  }
  ...
  if (c != nullptr) {
    ...
    *made_progress = true;
    ...
    NotifyOnCompactionCompleted(c->column_family_data(), c.get(), status, compaction_job_stats, job_context->job_id);
    if (marked_in_progress && compaction_input_level >= 0) {
      performance_monitor_.ClearCompactionInProgress(compaction_input_level);
      if (status.ok()) {
        if (performance_monitor_.IsForceCompaction(compaction_input_level)) {
          performance_monitor_.NotifyCompactionCompleted(compaction_input_level);
        }
      }
    }
