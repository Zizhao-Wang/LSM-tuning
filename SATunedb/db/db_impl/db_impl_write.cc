...
Status DBImpl::Write(const WriteOptions& write_options, WriteBatch* my_batch) {
  Status s;
  TUNER_PERF_TIMER_GUARD_WITH_NORMAL_TUNING(write,immutable_db_options_.clock);
...
void DBImpl::GetL0StallStats(int level, uint64_t* slowdown_micros, uint64_t* slowdown_count, uint64_t* stop_micros, uint64_t* stop_count) {
  ...
  if (cfd) {
    const auto& stats = cfd->GetLevelStallStats();
    *slowdown_micros = stats.total_slowdown_micros.load(std::memory_order_relaxed);
    *slowdown_count = stats.total_slowdown_count.load(std::memory_order_relaxed);
    *stop_micros = stats.total_stop_micros.load(std::memory_order_relaxed);
    *stop_count = stats.total_stop_count.load(std::memory_order_relaxed);
  } else {
    *slowdown_micros = 0;
    *slowdown_count = 0;
    *stop_micros = 0;
...
// ============================================================================
// SATune: Print all per-level L0 stall statistics to stderr
// Used for debugging and performance analysis
// ============================================================================
void DBImpl::PrintL0StallStats() {
  fprintf(stderr, "\n");
  fprintf(stderr, "========================================\n");
  fprintf(stderr, "SATune: L0 Stall Statistics\n");
  fprintf(stderr, "========================================\n");
    
  for (int level = 0; level < 7; ++level) {
    uint64_t slowdown_micros, slowdown_count, stop_micros, stop_count;
    GetL0StallStats(level, &slowdown_micros, &slowdown_count,&stop_micros, &stop_count);
  
    if (slowdown_count > 0 || stop_count > 0) {
      fprintf(stderr, "Level %d: slowdown=%lu times (%.2f ms total), stop=%lu times (%.2f ms total)\n",
        level, slowdown_count, slowdown_micros / 1000.0, stop_count, stop_micros / 1000.0);
    }
  }
  fprintf(stderr, "========================================\n\n");
}
...
Status DBImpl::DelayWrite(uint64_t num_bytes, WriteThread& write_thread,
  ...
  // ============================================================================
  // SATune: Capture stall cause at the beginning of delay
  // This prevents misattribution if stall cause changes during the delay period
  // ============================================================================
  WriteStallCause initial_stall_cause = write_controller_.GetStallCause();
  ...
  if (delayed) {
    ...
    // ============================================================================
    // SATune: Record L0-caused stalls for correlation with read performance
    // Only track stalls caused by L0 file count to enable parameter tuning
    // ============================================================================
    if (initial_stall_cause == WriteStallCause::kL0FileCountLimit || initial_stall_cause ==WriteStallCause::kPendingCompactionBytes) {
      // SATune: Get default CF (single CF scenario for experiments)
      ColumnFamilyData* cfd = nullptr;
      if (versions_ && versions_->GetColumnFamilySet()) {
        cfd = versions_->GetColumnFamilySet()->GetDefault();
      }
      if (cfd) {
        int current_l0_files = write_controller_.GetL0FileCount();
        // SATune: Get L0 trigger from mutable options
        const MutableCFOptions* mutable_options = cfd->GetLatestMutableCFOptions();
        int l0_trigger = mutable_options->level0_file_num_compaction_trigger;
        // SATune: For L0 stalls, record to level 0
        // Future work: could track which level is being compacted
        // SATune: Record stall time and count for this level
        cfd->RecordLevelStall(time_delayed, write_controller_.IsStopped(),current_l0_files, l0_trigger);  // is_stop: true for stop, false for slowdown
      }
    }
