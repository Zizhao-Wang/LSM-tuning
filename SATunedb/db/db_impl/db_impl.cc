...
DBImpl::DBImpl(const DBOptions& options, const std::string& dbname,
    ...
    : dbname_(dbname),
      ...
      tuning_log_(immutable_db_options_.tuning_log),
      satune_predict_total_micros_(0),
      performance_monitor_(rocksdb::tuner_perf_context, tuning_log_),
      compaction_tuning_state_(),
  ...
  fprintf(stderr, "[DEBUG] Inside DBImpl Constructor:\n"); 
  // 1. 检查 DBImpl 自己的成员变量
  fprintf(stderr, "   -> DBImpl::tuning_log_ address: %p\n", tuning_log_.get());
  // 2. 检查 immutable_db_options_ 里的 (这是源头)
  fprintf(stderr, "   -> immutable_db_options_.tuning_log address: %p\n", immutable_db_options_.tuning_log.get());
...
Status DBImpl::Get(const ReadOptions& _read_options,
  ...
  value->Reset();
  int source_level = -2;
  TUNER_PERF_TIMER_GUARD_WITH_LEVEL(read, immutable_db_options_.clock);
  ...
  Status s = GetImpl(read_options, column_family, key, value, timestamp, &source_level);
  return s;
}

Status DBImpl::GetImpl(const ReadOptions& read_options,
                       ColumnFamilyHandle* column_family, const Slice& key,
                       PinnableSlice* value, std::string* timestamp, int* source_level) {
  GetImplOptions get_impl_options;
  get_impl_options.column_family = column_family;
  get_impl_options.value = value;
  get_impl_options.timestamp = timestamp;
  get_impl_options.source_level = source_level;  // 传递下去
...
Status DBImpl::GetImpl(const ReadOptions& read_options, const Slice& key,
  ...
  if (!skip_memtable) {
    ...
    if (get_impl_options.get_value) {
      if (sv->mem->Get(
        ...
        // ========== SATune：记录从 memtable 读取 ==========
        if (get_impl_options.source_level) {
          *get_impl_options.source_level = -1;  // -1 表示 memtable
        }
        // ==============================================
        ...
        // ========== SATune：记录从 immutable memtable 读取 ==========
        if (get_impl_options.source_level) {
          *get_impl_options.source_level = -1;  // -1 表示内存
        }
        // ======================================================
      ...
      if (sv->mem->Get(lkey, /*value=*/nullptr, /*columns=*/nullptr,
        ...
        // ========== SATune ==========
        if (get_impl_options.source_level) {
          *get_impl_options.source_level = -1;
        }
        // ========================                
        done = true;
        RecordTick(stats_, MEMTABLE_HIT);
  ...
  if (!done) {
    ...
    sv->current->Get(
        ...
        get_impl_options.get_value, get_impl_options.source_level);
...
Iterator* DBImpl::NewIterator(const ReadOptions& _read_options,
   ...
   TUNER_PERF_TIMER_GUARD(seek,immutable_db_options_.clock);                              
  if (_read_options.io_activity != Env::IOActivity::kUnknown &&
