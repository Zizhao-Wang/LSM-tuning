...
void Version::Get(const ReadOptions& read_options, const LookupKey& k,
                  ...
                  bool* is_blob, bool do_merge,int* source_level) {
  ...
  while (f != nullptr) {
    ...
    *status = table_cache_->Get(
    ...
    //============SATune============
    uint64_t elapsed = clock_->NowMicros() - start;
    ROCKSDB_NAMESPACE::tuner_perf_context.level_stats[fp.GetHitFileLevel()].read_micros.fetch_add(elapsed);
    ROCKSDB_NAMESPACE::tuner_perf_context.level_stats[fp.GetHitFileLevel()].LG_tuning_read_micros.fetch_add(elapsed);
    ROCKSDB_NAMESPACE::tuner_perf_context.level_stats[fp.GetHitFileLevel()].read_count.fetch_add(1);
    ROCKSDB_NAMESPACE::tuner_perf_context.level_stats[fp.GetHitFileLevel()].LG_tuning_read_count.fetch_add(1);
    //==============SATune============
    ...
    switch (get_context.State()) {
      ...
      case GetContext::kFound:
        if (source_level) {
          *source_level = fp.GetHitFileLevel();  // 0, 1, 2, ... 6
        }
      ...
      if (compaction_style_ == kCompactionStyleFIFO) {
        ...
        if (compaction_style_ == kCompactionStyleLevel && num_levels() > 1) {
          ...
          if (immutable_options.level_compaction_dynamic_level_bytes) {
            ...
            double size_score = static_cast<double>(total_size) /mutable_cf_options.max_bytes_for_level_base;
            // if (score >= 1.0 || size_score >= 1.0) {
            //   fprintf(stdout, "[SATune-DEBUG] L0-Score Check: Final=%.2f | ByCount: %d/%d=%.2f | BySize: %lu/%lu=%.2f | Winner: %s\n",
            //     std::max(score, size_score),num_sorted_runs, mutable_cf_options.level0_file_num_compaction_trigger, score, 
            //     (unsigned long)total_size, (unsigned long)mutable_cf_options.max_bytes_for_level_base, size_score, 
            //     (size_score > score ? "SIZE (Size limit hit!)" : "COUNT (File count limit hit!)"));
            //   fflush(stdout);
            // }
            // 3. 原有逻辑：取最大值作为最终分数
