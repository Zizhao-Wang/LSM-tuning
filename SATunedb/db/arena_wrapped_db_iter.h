...
class ArenaWrappedDBIter : public Iterator {
 public:
  ...
  bool Valid() const override { TUNER_PERF_TIMER_GUARD_WITH_NORMAL_TUNING(seek, clock_); return db_iter_->Valid(); }
  void SeekToFirst() override { db_iter_->SeekToFirst(); }
  void SeekToLast() override { db_iter_->SeekToLast(); }
  // 'target' does not contain timestamp, even if user timestamp feature is
  // enabled.
  void Seek(const Slice& target) override { TUNER_PERF_TIMER_GUARD_WITH_NORMAL_TUNING(seek,clock_); db_iter_->Seek(target); }
  void SeekForPrev(const Slice& target) override {
    db_iter_->SeekForPrev(target);
  }
  void Next() override { TUNER_PERF_TIMER_GUARD_WITH_COUNT_AND_NORMAL_TUNING(seek, seek_length_count, clock_); db_iter_->Next(); }
  void Prev() override { TUNER_PERF_TIMER_GUARD_WITH_COUNT_AND_NORMAL_TUNING(seek, seek_length_count, clock_); db_iter_->Prev(); }
